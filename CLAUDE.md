...
stale selector (`appliqation-heal-selector`), check for a visual regression
(`appliqation-visual-regression`), fix a defect (`appliqation-defect-fix`), generate
new automation (`appliqation-scriptgen`), run a headless exploratory-QA pass
...
The seven sibling agents (`appliqation-autotest`/`appliqation-defect-fix`/
`appliqation-scriptgen`/`appliqation-pr-raise`/`appliqation-explorer`/
`appliqation-heal-selector`/`appliqation-visual-regression`) are the opposite: genuinely
...
The same reasoning-belongs-in-Autopilot logic applies to `run_heal` and
`run_visual_check`: `appliqation-heal-selector` only ever sees one script and one
failure in isolation, and `appliqation-visual-regression` only ever sees one route and
one screenshot pair in isolation. Neither has any way to know whether this TC's
failure is a stale-selector candidate versus a real regression, or whether this page is
business-critical enough to warrant a visual check at all, or how either fits into a
scenario/test-set's relative priority. That first-verify-then-decide judgment (heal
before defect-fix, a visual check only for a TC tagged `visual` or an equally specific
stated reason, a decline or an inconclusive/expected-divergence verdict is real
evidence, not a failure) lives entirely in this repo's policy, not in either sibling
agent itself.
...
- **No filesystem or private-npm dependencies on the sibling repos.** They're invoked
  ...
  `DEFECT_FIX_CMD`/`EXPLORER_CMD`/`HEAL_CMD`/`VISUAL_CMD`, see `src/config/env.ts`),
  spawned via `child_process.execFile`, never imported. An external clone with no
  access to `~/Sites/localhost/appliqation-autotest/` etc. still works once the
  sibling packages are published to npm (defaults assume they're on PATH) or if the
  user points the env vars at wherever they've built them.
...
- The README links all seven sibling repos at their real `github.com/appliqation/...`
  URLs. Keep that list in sync (table + diagram) whenever a sibling agent is added,
  renamed, or removed.

## Where to find what

- `src/policy/systemPrompt.ts` — **the actual customization point.**
  `buildSystemPrompt(allowPr, allowVisual)` is the full decision methodology:
  context-gathering priorities (current state first, then flakiness/defects/coverage/
  quality), the decision rules (**defect/TC mismatch checked first**: a defect linked
  to a test case that doesn't actually describe it (common for exploratory defects
  filed against whatever TC was active, not one written for that issue) routes
  straight to `run_defect_fix` regardless of the mismatched TC's own pass/fail status,
  never `run_judge`/`run_generate` against the wrong thing; no evidence leads to
  judging first; **failing + canonical script exists leads to `run_heal` first** (a
  ...
  failing + no canonical means don't generate; passing + no canonical means generate;
  flaky means generate with lower confidence; low-priority means recommend and stop,
  that's a legitimate outcome), the adaptive-execution discipline (re-check every real
  result, never execute a plan blindly), and the report structure (every claim must
  cite a real tool result). `run_explore`'s guidance is deliberately NOT a
  deterministic trigger list. See its own paragraph in Phase 2: the bar is a specific,
  stated reason drawn from this TC's own gathered context (a fragility signal, a
  recurring known_issues category, a new UI component), with three illustrative (not
  exhaustive) examples and an explicit counterweight against calling it reflexively,
  matching this agent's own foundational "genuine judgment, not a rule checklist"
  design, not a special case for this one tool. `run_visual_check`'s own paragraph in
  Phase 2 follows the identical shape: only reachable when `--visual` authorized it,
  only actually called for a TC tagged `visual` or an equally specific stated reason,
  never routinely; its four possible verdicts (`regression`/`expected-divergence`/
  `not-applicable`/`inconclusive`) are each given explicit handling in Phase 3, and
  `secondaryFindings` are reported but never auto-escalated on their own.
  **Scope-aware, not single-TC-only:** Phase 1 splits into a single-TC path (start with
  ...
  `get_test_set`, then lead with exactly **one scope-level `run_judge` call**, with
  ...
  Phase 1 also notes that `run_visual_check`'s real route comes from
  `get_execution_evidence` on a `run_id` already in hand, never guessed from step text.
  Phase 2 gains a "prioritize across TCs" bullet for scope mode (rank by the same
  signals used per-TC: `high_risk_areas`, `known_issues`, `defect_history`, across the
  whole scope, not list order). Phase 4's report structure branches too: single-TC
...
- `src/orchestrator/autopilot.ts` — `autopilot()`: builds the tool palette (context
  ...
  call**, explicitly never to loop calling `run_judge` per test case itself. An
  optional `baselineEnvironment` field, when given, is stated plainly in the seed
  message too (production/baseline environment name for `run_visual_check`; without
  it there is nothing for the model to pass as `baseline_environment` even if the tool
  is otherwise available). Still exactly one `runLoop()` call regardless of scope, no
  code-level per-TC looping in this file; the per-TC reasoning happens inside that one
  LLM session.
- `src/tools/metaTools.ts` — `run_judge`/`run_generate`/`run_defect_fix`/`run_explore`/
  `run_heal`/`run_pr_raise`/`run_visual_check` as `LlmToolDef`s + a dispatcher. Each
  spawns the configured sibling command with its real CLI flags plus `--json`, and
  returns the sibling's actual structured output as the tool result, including on
  failure (a failed/blocked judge, an unverified generate/defect-fix/heal, a
  budget-exceeded explore, a regression/inconclusive visual check), recovered from
  stdout even though the process exits non-zero, since every sibling CLI prints its
  `--json` summary before setting its exit code. `run_judge`'s schema now only
  ...
  `defect_id`), spawned via `healCmd` (a `MetaToolsConfig` field, following the exact
  ...
  `verified`/`declined` verdict, not the model's claim. `run_visual_check` wraps
  `appliqation-visual-regression check` (required: `test_case_uuid`, `route`,
  `baseline_environment`, `target_environment`; optional `mask` array, repeated as one
  `--mask` per entry, and `storage_state`), spawned via `visualCmd`, and is gated
  exactly like `run_pr_raise` (see below), not unconditional like `run_heal`: pushed
  into `metaToolDefs()`'s returned array only `if (cfg.allowVisual)`, and the
  dispatcher case re-checks `cfg.allowVisual` again as a second gate.
  `run_defect_fix`'s schema marks `test_instruction` **required**: the model must
  state, from its own gathered evidence, what testing scope the fix needs verified
  (see the policy's Phase 2); `run_explore` has no such required field but its
  description and the policy's Phase 2 both require the model to compose its `prompt`
  argument from a real, stated reason, not a generic instruction. `run_defect_fix`,
  `run_explore`, and `run_heal` are always offered regardless of `allowPr`/
  `allowVisual`, since none of them touch git/GitHub; unlike `run_pr_raise`/
  `run_visual_check`, none of them have a `dry_run`/authorization gate either
  (`run_explore` has no real write path to suppress in the first place, see
  `PROJECT_CONTEXT_TOOL` below). `metaToolDefs()`'s `run_pr_raise` entry is only
  present when `allowPr` is true, and `run_visual_check`'s entry only when
  `allowVisual` is true: hardcoded exclusions from what's offered to the model, not a
...
- `src/tools/safety.ts` — `READONLY_CONTEXT_TOOLS`, this agent's own read-only appq
  ...
  decision needs the same breadth of signal a senior engineer would look at).
  `get_execution_evidence` is in this set, it's where `run_visual_check`'s real route
  comes from. The enforcement mechanism (`assertToolAllowed`/
  `createGatedAppqDispatcher`) is shared from `@appliqation/agent-core`, same as every
  sibling agent. `enrich_project_context` is kept **out** of `READONLY_CONTEXT_TOOLS`
  on purpose: that set's whole guarantee is "no write capability under any argument,"
  and this one tool has a write mode, so it needs the separate gate below instead of
  blending into a set that promises something it can't keep for this one entry.
...
- `src/cli/index.ts` — the `run` command. `--allow-pr` gates `run_pr_raise`; `--visual`
  (together with a required `--baseline-environment <name>`, validated before any API
  call) gates `run_visual_check` the same way; `--policy <path>` overrides the bundled
  system prompt; exactly one of `--test-case-uuid <uuid>` / `--scenario-id <id>` /
  `--test-set-id <id>` is required (validated at the top of the action handler, prints
  an error and sets a non-zero exit code without ever calling `autopilot()`
  otherwise); project_id/scenario_id are never CLI inputs at all here, the model
  derives them itself from `get_scenario` during Phase 1 of its own reasoning, same as
  every write call downstream (`run_pr_raise`'s `project_id` arg comes from what the
  model read, not from anything this CLI resolves).
- `src/cli/audit.ts` — `recordAutopilotRun()`, extracted out of `cli/index.ts` for the
  ...
  includes `allowPr` and `allowVisual` alongside the usual `LoopResult` fields, plus a
  `scope` object (`testCaseUuid`/`scenarioId`/`testSetId`, whichever was given):
  whether a PR or a visual check was even authorized, and exactly what scope this
  invocation covered, are all part of the record.
- `src/config/env.ts` — appq connection (context tools only), LLM provider config, the
  seven `*_CMD` sibling-invocation strings (including `healCmd`, default
  `appliqation-heal-selector`, and `visualCmd`, default
  `appliqation-visual-regression`), budget, `POLICY_FILE`. `auditSink` resolves
  `AUDIT_MONGO_*`/`AUDIT_JSONL_PATH` via `@appliqation/agent-core/audit`'s
  `resolveAuditSink()`, opt-in, no-op when unconfigured.

## Commands

- `npm run dev -- run --test-case-uuid <uuid> --environment <name> --repo-path <path> [--allow-pr] [--visual --baseline-environment <name>] [--policy <path>] [--json|--ci]`
...
use). The seven `*_CMD` vars need to actually resolve to something runnable, either
...
check that the README's description of it stays accurate. External readers judge this
