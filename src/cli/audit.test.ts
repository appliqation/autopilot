...
describe('recordAutopilotRun', () => {
  it('records one call with agent/subcommand and the outcome including allowPr/allowVisual', async () => {
    ...
    expect(record.outcome).toEqual({ testCaseUuid: '2424-abc', environment: 'Stage', repoPath: '/repo', allowPr: true, allowVisual: true, turns: 7, budgetExceeded: false, report: 'done' });
  });

  it('records exitCode 1 and an error outcome when result is undefined, autopilot() threw', async () => {
    const sink: AuditSink = { record: vi.fn().mockResolvedValue(undefined), close: vi.fn().mockResolvedValue(undefined) };
    await recordAutopilotRun({ sink, startedAt: 0, endedAt: 1, model: 'x', usage, testCaseUuid: 'tc-1', environment: 'Stage', repoPath: '/repo', allowPr: false, allowVisual: false, result: undefined });
    const record = (sink.record as ReturnType<typeof vi.fn>).mock.calls[0][0];
    expect(record.exitCode).toBe(1);
    expect(record.outcome).toEqual({ testCaseUuid: 'tc-1', environment: 'Stage', repoPath: '/repo', allowPr: false, allowVisual: false, error: true });
  });
